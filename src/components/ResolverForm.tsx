...
export const ResolverForm = ({ onResult, isLoading, setIsLoading }: ResolverFormProps) => {
  ...
  const handleSampleClick = async (sampleAlias: string) => {
    setAlias(sampleAlias);
    setIsLoading(true);

    try {
      const { data, error } = await supabase.functions.invoke('resolve-alias', {
        body: { alias: sampleAlias, chain }
      });
      ...
      if (data.errorMessage) {
        ...
        toast({
          title: "Success",
          description: `Resolved ${sampleAlias} to ${data.resolvedAddress}`,
        });
      }
    } catch (error) {
      console.error('Resolution error:', error);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <Label className="text-sm font-medium">Sample Aliases</Label>
